...
RUN scl enable rh-ruby23 'gem install --no-document fluent-plugin-kubernetes_metadata_filter -v 0.25.3' && \
    scl enable rh-ruby23 'gem install --no-document fluent-plugin-elasticsearch -v 1.6.0' && \
    scl enable rh-ruby23 'gem install --no-document fluent-plugin-prometheus -v 0.2.0' && \
